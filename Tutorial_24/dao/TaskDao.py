...
class TaskDao:
    ...
    def __init__(self):
        ...
        self.list = [
            Task(1, "Clean Room", datetime(2024, 2, 1), "Home", False),
            Task(2, "Do Homework", datetime(2024, 2, 2), "School", True),
    ...
    def create_task(self, taskName, taskCategory):
        # grabbing the largest ID so we can increment the id ourselfs
        new_id = max(task.taskId for task in self.list) + 1 if self.list else 1
        new_task = Task(new_id, taskName, datetime.now(), taskCategory, False)
